package org.firstinspires.ftc.teamcode.photonreborn;
...
public class PhotonCore implements Runnable, OpModeManagerNotifier.Notifications {
    ...
    public void onOpModePreInit(OpMode opMode) {
        ...
        if(!opMode.getClass().isAnnotationPresent(Photon.class))
            return;
        ...
        for(LynxModule m : replacements.keySet()){
            ...
            try {
                addConfiguredModule(usbDevice,replacements.get(m));
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    private LynxModule addConfiguredModule(LynxUsbDevice usbDevice, LynxModule oldModule) throws InterruptedException, RobotCoreException {
        LynxModuleDescription d = new LynxModuleDescription.Builder(
                oldModule.getModuleAddress(),
                oldModule.isParent()
        ).build();

        if(oldModule.isSystemSynthetic()){
            try {
                Field field = LynxModuleDescription.class.getDeclaredField("isSystemSynthetic");
        ...
        return usbDevice.getOrAddModule(d);
    }
